...
use binary_sidecar::{
    deps::{
        ffmpeg::FfmpegFetcher, ytdlp::YtdlpFetcher, ReleaseFetcher
    }, download_and_extract_binary, utils::{architecture::Architecture, platform::Platform}
};
...
async fn main() {
    ...
    let ffmpeg_release = ffmpeg_fetcher.get_release(&platform, &architecture).await.unwrap();
