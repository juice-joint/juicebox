...
use crate::server::utils::yt_searcher::{SearchError, SearchResult, YtSearcher};
...
impl VideoSearcherActor {
    ...
    async fn handle_message(&mut self, msg: VideoSearcherActorMessage) {
        ...
        match msg {
            VideoSearcherActorMessage::SearchVideo { query, respond_to } => {
                info!(
                    "Consumer {} starting to process search query {}",
                ...
                info!(
                    "Consumer {} finished searching for {} result {}",
                    self.consumer_id,
                    query,
...
async fn run_video_searcher_actor(mut actor: VideoSearcherActor) {
    info!(
        "Starting video searcher actor consumer {}",
        actor.consumer_id
    );
    loop {
        trace!(
...
impl VideoSearcherActorHandle {
    ...
    pub async fn search_videos(&self, query: &str) -> Result<Vec<SearchResult>, SearchError> {
        trace!(
            "Requesting searches for {} (channel len: {})",
            query,
            self.sender.len()
        );
        ...
        trace!(
            "Sending search request {} to video searcher actor (channel len: {})",
            query,
            self.sender.len()
        );
